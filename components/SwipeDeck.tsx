...
export function SwipeDeck({ cards, voter, onDone }: { cards: Card[]; voter: string; onDone?: () => void }) {
  const steps: Step[] = useMemo(
    () =>
      cards.flatMap((card, cardIdx) => {
        const variants = ALL_VARIANT_ORDER.filter(
          (v) => card.after[v] != null
        );
        return variants.map((variant, variantIdx) => ({ card, variant, cardIdx, variantIdx }));
      }),
    [cards]
  );
  ...
  if (!step) {
    return <Done shipped={shipped} killed={killed} needsWork={needsWork} total={total} voter={voter} onBack={onDone} />;
  }
      ...
      <div className="flex flex-col gap-1.5 px-4 pt-4 pb-3 shrink-0 border-b border-white/5 bg-ink-950/80 backdrop-blur">
        ...
        <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-ink-400">
          ...
          <span className="inline-flex items-center gap-1.5">
            ...
            <span className="text-ink-500">· variant {step.variantIdx + 1}</span>
          </span>
        </div>
      </div>
...
function Done({ shipped, killed, needsWork, total, voter, onBack }: { shipped: number; killed: number; needsWork: number; total: number; voter: string; onBack?: () => void }) {
  ...
  return (
    <div className="min-h-[100dvh] grid place-items-center px-6 py-12">
      <div className="max-w-md w-full text-center">
        ...
        <p className="mt-2 text-[14px] text-ink-300">All {total} variants reviewed across {CARDS.length} cards. The Customer.io rebuild starts from your feedback.</p>
        ...
        <div className="mt-8 flex items-center justify-center gap-3">
          {onBack && (
            <button
              onClick={onBack}
              className="h-11 inline-flex items-center justify-center px-6 rounded-xl border border-white/10 text-ink-200 font-semibold text-[14px] hover:bg-white/[0.04] transition"
            >
              ← Back to decks
            </button>
          )}
